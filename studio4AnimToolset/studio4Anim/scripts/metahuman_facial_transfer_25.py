# Metahuman Facial Animation Transfer (Maya 2025 port — maya.cmds, no PyMEL)
#
...
import metahuman_api_25 as mh_api
...
class UI(MayaQWidgetBaseMixin, QtWidgets.QWidget):
	...
	def _set_metahuman_name(self):
		...
		if not selected:
			...
			msg_box.exec_()
			return
		current_namespace = mh_api.get_namespace(selected[0])
		embedded_nodes = cmds.ls(type='embeddedNodeRL4')
		matching_nodes = [node for node in embedded_nodes if mh_api.get_namespace(node) == current_namespace]
		if not matching_nodes:
			...
			msg_box.exec_()
			return
		embedded_node = matching_nodes[0]
		file_path = cmds.getAttr('{}.dnaFilePath'.format(embedded_node))
		...
		self._current_namespace = mh_api.get_namespace(embedded_node)
		cmds.namespace(setNamespace=self._current_namespace if self._current_namespace else ':')
	...
	def import_metahuman_animation(self, transfer_type):
		...
		file_path = cmds.fileDialog2(fileFilter='(FBX (*.fbx)',
		                             fileMode=1,
