"""
teamHope Uninstaller
======================
Drag and drop this file onto the Maya viewport to remove all files and
shelves that were installed by install.py.

If versioned backups exist you're offered a choice:
        ...
        for fname in os.listdir(os.path.join(backup_dir, "shelves")):
            if fname.startswith("shelf_") and fname.endswith(".mel"):
                ...
                try:
                    ...
                    cmds.warning(
                        "teamHope: shelf file restored but could not "
...
def _uninstall():
    ...
    if not os.path.isfile(manifest):
        cmds.confirmDialog(
            title="teamHope Uninstaller",
            message="Nothing to uninstall — no manifest found.\n\n"
                    "teamHope does not appear to be installed.",
            button=["OK"],
        )
        return
    ...
    if len(versions) >= 2:
        ...
        choice = cmds.confirmDialog(
            title="teamHope Uninstaller",
            message="This will remove all teamHope scripts, icons, and shelves.\n\n"
    ...
    if restore_version is not None:
        files_restored = _restore_from(restore_version[1])
    ...
    if files_restored:
        summary += (
            ...
            "    {} file(s)"
        ).format(restore_version[0], files_restored)
    if errors:
        summary += "\n\n  Errors:\n    " + "\n    ".join(errors)
