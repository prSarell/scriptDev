...
class ShotSub(object):
    ...
    def write_shotgrid_link(self, sg_project_id, sg_project_name, entity_type, entity_id, entity_code):
        ...
        try:
            import jiffySG
            linked_by = jiffySG.current_login()
        except Exception:
            pass
    ...
    def link_to_shotgrid(self):
        ...
        during an occasional, deliberate "link" action is fine, and this
        avoids replicating jiffySG's documented main-thread-only
        maya.cmds hazard for no real benefit.
        """
        try:
            import jiffySG
        except ImportError:
            cmds.warning(
                "Jiffy SG isn't available in this session — it owns the ShotGrid "
                "connection and isn't on Maya's default script path (dev/test only). "
                "Run its dev launcher first, then try again."
            )
            return

        try:
            projects = jiffySG.list_visible_projects()
        except Exception as exc:
        ...
        try:
            shots = jiffySG.list_project_shots(project_name)
        except Exception as exc:
            cmds.warning("Could not reach ShotGrid to list Shots: {0}".format(exc))
            return
        if not shots:
            cmds.warning(
                "No Shots found in ShotGrid Project '{0}' — create one in "
                "Jiffy SG's Shots tab first.".format(project_name)
            )
            return
