...
4. **`create_shot()` implemented and verified, 2026-07-24** — first real write path beyond auth. `create_shot(shot_code, project_name, sequence_name=None, as_login=None)` looks up the Project by name, optionally finds-or-creates a ShotGrid `Sequence` (via `_find_or_create_sequence()`), then finds-or-creates a `Shot` linked to it (`Shot.sg_sequence`) — idempotent, safe to call repeatedly. Deliberately minimal: no Task/status/frame-range fields yet, since Task status codes aren't set up on any real Project (see item 3).
   ...
   - Next actual code piece: implement `upload_playblast()` for real (currently `NotImplementedError`) — needs the Task status codes above decided first, plus a decision on Shot/Task lookup (by name match against `VART3575`?) before it can create Version entities.
5. Pilot with the one existing 3-person team first — lower stakes, easier to debug auth/API rough edges before wider rollout.
6. Roll out to the rest of the class, each with their own ShotGrid Project (cloned from a template if useful, since ShotGrid's project-duplicate feature is admin-console-only and can't be triggered from a student's API session).
7. Once proven, retire JiffySchedule in favour of Jiffy SG.
